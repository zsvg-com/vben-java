...
public class DemoLinkMainController {
    ...
    @SaCheckPermission("link:main:query")
    @GetMapping("info/{id}")
    public R<DemoLinkMain> info(@PathVariable Long id) {
        DemoLinkMain main = service.select(id);
        return R.ok(main);
    }
    ...
    @Log(title = "关联主表案例", businessType = BusinessType.INSERT)
    ...
    @Log(title = "关联主表案例", businessType = BusinessType.UPDATE)
    @RepeatSubmit
    @PutMapping
    ...
    @Log(title = "关联主表案例", businessType = BusinessType.DELETE)
    @DeleteMapping("{ids}")
