package vben.common.mybatis.handler;
...
import vben.common.core.domain.dto.RoleDTO;
...
import vben.common.mybatis.annotation.DataColumn;
import vben.common.mybatis.annotation.DataPermission;
import vben.common.mybatis.enums.DataScopeType;
import vben.common.mybatis.helper.DataPermissionHelper;
...
public class PlusDataPermissionHandler {
    ...
    public Expression getSqlSegment(Expression where, boolean isSelect) {
        try {
            ...
            if (StrUtils.isBlank(dataFilterSql)) {
                return where;
            }
    ...
    private String buildDataFilter(DataPermission dataPermission, boolean isSelect) {
        ...
        if (StrUtils.isNotBlank(dataPermission.joinStr())) {
        ...
        for (DataColumn dataColumn : dataPermission.value()) {
            ...
            // 包含权限标识符 这直接跳过
            if (StrUtils.isNotBlank(dataColumn.permission()) &&
        ...
        for (RoleDTO role : user.getRoles()) {
            ...
            for (DataColumn dataColumn : dataPermission.value()) {
                ...
                // 不包含 key 变量 则不处理
                if (!StrUtils.containsAny(type.getSqlTemplate(), keys.toArray(String[]::new))) {
                    continue;
                }
                // 当前注解不满足模板 不处理
                if (!StrUtils.containsAny(type.getSqlTemplate(), dataColumn.key())) {
                    continue;
                }
            ...
            // 未处理成功则填充兜底方案
            if (!isSuccess && StrUtils.isNotBlank(type.getElseSql())) {
