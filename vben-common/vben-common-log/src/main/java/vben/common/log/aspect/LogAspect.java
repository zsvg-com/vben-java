package vben.common.log.aspect;
...
import vben.common.log.enums.BusinessStatus;
...
//    private static final ThreadLocal<StopWatch> KEY_CACHE = new ThreadLocal<>();
    ...
    private static final ThreadLocal<TimeInterval> KEY_CACHE2 = new ThreadLocal<>();
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            if (e != null) {
                ...
                operLog.setErrorMsg(StrUtils.sub(e.getMessage(), 0, 3800));
            }
            // 设置方法名称
...
//            operLog.setCostTime(stopWatch.getDuration().toMillis());
            ...
            operLog.setCostTime(timer.interval());
            // 发布事件保存数据库
...
//            KEY_CACHE.remove();
    ...
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, OperLogEvent operLog, Object jsonResult) throws Exception {
        ...
        // 是否需要保存response，参数和值
        if (log.isSaveResponseData() && ObjectUtils.isNotNull(jsonResult)) {
            operLog.setJsonResult(StrUtils.sub(JsonUtils.toJsonString(jsonResult), 0, 3800));
        }
    }
    ...
    private void setRequestValue(JoinPoint joinPoint, OperLogEvent operLog, String[] excludeParamNames) throws Exception {
        ...
        if (MapUtil.isEmpty(paramsMap) && StrUtils.equalsAny(requestMethod, HttpMethod.PUT.name(), HttpMethod.POST.name(), HttpMethod.DELETE.name())) {
            String params = argsArrayToString(joinPoint.getArgs(), excludeParamNames);
            operLog.setOperParam(StrUtils.sub(params, 0, 3800));
        } else {
            MapUtil.removeAny(paramsMap, EXCLUDE_PROPERTIES);
            MapUtil.removeAny(paramsMap, excludeParamNames);
            operLog.setOperParam(StrUtils.sub(JsonUtils.toJsonString(paramsMap), 0, 3800));
        }
    }
