package vben.common.excel.core;
...
import vben.common.excel.annotation.ExcelEnumFormat;
...
public class ExcelDownHandler implements SheetWriteHandler {
        ...
        for (Map.Entry<Integer, FieldWrapper> entry : fieldCache.getSortedFieldMap().entrySet()) {
            ...
            if (field.isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                if (StrUtils.isNotBlank(dictType)) {
                    ...
                    // 如果指定了确切的值，则直接解析确切的值
                    List<String> strList = StrUtils.split(converterExp, format.separator());
                    options = StreamUtils.toList(strList, s -> StrUtils.splitToArray(s, "=")[1]);
                }
    ...
    private void dropDownLinkedOptions(DataValidationHelper helper, Workbook workbook, Sheet sheet, DropDownOptions options) {
        ...
        columnValueMap.forEach((rowIndex, rowValues) -> {
            ...
            for (int columnIndex = 0; columnIndex < rowValues.size(); columnIndex++) {
                ...
                // 填充位置的部分不渲染
                if (StrUtils.isNotBlank(rowValue)) {
    ...
    private String getExcelColumnName(int columnIndex) {
        ...
        String columnPrefix = columnCircleCount == 0
            ? StrUtils.EMPTY
            : StrUtils.subWithLength(EXCEL_COLUMN_NAME, columnCircleCount - 1, 1);
        // 从26一循环内取对应的栏位名
