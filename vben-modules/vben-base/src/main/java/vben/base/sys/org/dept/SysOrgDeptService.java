...
public class SysOrgDeptService {
    ...
    private final SysOrgDeptDao deptDao;

    private final OrgDao orgDao;

    @Transactional(readOnly = true)
    public List<SysOrgDeptVo> findVoList(Sqler sqler) {
        return jdbcHelper.getTp().query(sqler.getSql(),
            new BeanPropertyRowMapper<>(SysOrgDeptVo.class), sqler.getParams());
    }
    ...
    public void insert(SysOrgDept main) {
        if (main.getId() == null) {
    ...
    public void update(SysOrgDept main) {
        if (main.getOrnum() == null) {
            main.setOrnum(0);
        }
        ...
        Org org = new Org(main.getId(), main.getName(), 1);
        deptDao.update(main);
        String oldTier = jdbcHelper.findString("select tier from sys_org_dept where id=?", main.getId());
    ...
    public void move(Smove bo) {
        SysOrgDept dragDept = deptDao.findById(bo.getDraid());
        String sql = "select id,ornum from sys_org_dept where ornum>? and pid=?";
        ...
        String updateSql = "update sys_org_dept set ornum=? where id=?";
        List<Object[]> updateList = new ArrayList<>();
        ...
        if ("inner".equals(bo.getType())) {
            ...
            SysOrgDept dropDept = deptDao.findById(bo.getDroid());
            dragDept.setPid(dropDept.getPid());
            dragDept.setOrnum(dropDept.getOrnum());
            String sql3 = "select id,ornum from sys_org_dept where ornum>? and pid=?";
            ...
            String updateSql3 = "update sys_org_dept set ornum=? where id=?";
            ...
            String updateSql4 = "update sys_org_dept set ornum=? where id=?";
