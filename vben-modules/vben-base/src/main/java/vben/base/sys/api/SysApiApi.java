...
public class SysApiApi {
    ...
    @SaCheckPermission("sys:api:query")
    @GetMapping("info/{id}")
    public R<SysApi> one(@PathVariable Long id) {
        SysApi main=service.findById(id);
        return R.ok(main);
    }
    ...
    @PostMapping
    public R<Long> post(@RequestBody SysApi main) {
        main.setAvtag(true);
        service.insert(main);
    ...
    @SaCheckPermission("sys:api:edit")
    @PutMapping
    public R<Long> put(@RequestBody SysApi main) {
