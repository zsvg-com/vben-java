...
public class BpmOrgTreeCateDao {

    public List<Stree> findTreeList(String id) {
        if(StrUtils.isBlank(id)) {
            return jdbcHelper.findStreeList("select id,name,pid from bpm_org_tree_cate order by ornum");
        }else{
            return jdbcHelper.findStreeList("select id,name,pid from bpm_org_tree_cate where id <> ? order by ornum",id);
        }
    }

    public BpmOrgTreeCate findById(String id) {
        String sql = "select * from bpm_org_tree_cate where id = ?";
        return jdbcHelper.getTp().queryForObject(sql, new BeanPropertyRowMapper<>(BpmOrgTreeCate.class), id);
    }

    public void insert(BpmOrgTreeCate cate) {
        Isqler isqler = new Isqler("bpm_org_tree_cate");
    ...
    public void update(BpmOrgTreeCate cate) {
        Usqler usqler = new Usqler("bpm_org_tree_cate");
    ...
    public void deleteById(String id) {
        String sql = "delete from bpm_org_tree_cate where id=?";
        jdbcHelper.getTp().update(sql, id);
    }

    public Integer getCount(String pid){
        String countSql="select count(1) from bpm_org_tree_cate where pid=?";
